...
Although we focus on arc length discrepancies as the primary failure mode (described in \Cref{sec:ast_application}), this work can be easily extended to other failure events.
...
\citeauthor*{ast_traj_plan} applied AST to trajectory planning systems, but as will be discussed further in \Cref{sec:ast_approach_ast}, that problem had access to the full FMS, thus fitting the model of traditional AST.
...
Adaptive stress testing (AST) is a black-box approach to find rare failure events in cyber-physical systems \cite{ast_acasx,lee2020adaptive}.
...
% Julia is an open-source, high-level, dynamically-typed, scientific computing language that can achieve speeds comparable to C \cite{bezanson2017julia}.
...
\begin{figure*}[!t]
  \centering
    ...
    \resizebox{0.47\textwidth}{!}{\input{figures/episodic_ast/dasc-failure.pgf}}
    ...
    \resizebox{0.47\textwidth}{!}{\input{figures/episodic_ast/dasc-failure-zoomed.pgf}}
